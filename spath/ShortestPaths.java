...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	while(!pq.isEmpty()) {
    		VertexAndDist vert = pq.extractMin();
    		Vertex v = vert.vertex;
    		Iterable<Edge> edges;
    		edges=v.edgesFrom();
    		for(Edge e: edges){			
    			int change_dist=weights.get(e)+vert.distance;
    			Decreaser<VertexAndDist> d=handles.get(e.to);
				VertexAndDist new_d = d.getValue();
				int now_dist= new_d.distance;
				if(change_dist<now_dist){
					d.decrease(new VertexAndDist(new_d.vertex, change_dist));
					parentEdges.put(new_d.vertex, e);
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	Vertex to_vert;
    	to_vert = endVertex;
    	while(to_vert != startVertex) {
    		path.addFirst(parentEdges.get(to_vert));
    		to_vert = parentEdges.get(to_vert).from;
    	}
